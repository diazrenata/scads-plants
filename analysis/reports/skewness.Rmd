...
skew_hist <- ggplot(data = filter(ranked_skew_df, source == "observed"), aes(x = season, y= skew_percentile,color = season)) +
  geom_violin() +
...
s_n <- ranked_skew_long  %>%
  filter(source == "observed") %>%
  select(year, season, rank, abund) %>%
  group_by(year, season) %>%
  summarize(s = max(rank), n = sum(abund)) %>%
  ungroup()

ranked_skew_df <- ranked_skew_df %>%
  left_join(s_n, by = c("year", "season"))

p_s_n_plot <- ggplot(data = filter(ranked_skew_df, source == "observed"), aes(x = n, y = s, color = skew_percentile)) +
  geom_point() +
  geom_point(data = filter(ranked_skew_df, skew_percentile <= 75, source == "observed"), aes(x = n, y = s, color = skew_percentile), size =3) +
  theme_bw() +
...
uskew <- left_join(uskew, filter(ranked_skew_df, source == "observed"), by = c("year", "season", "treatment"))

sims_p_plot <- ggplot(data = uskew, aes(x = n_elements, y = skew_percentile, color = skew_percentile)) +
  geom_point() +
  theme_bw() +
    scale_color_viridis_c(option = "plasma", end = .7)
...
```{r heatmaps, fig.height = 100, fig.width = 10}
...
```{r obs S and N, fig.height = 5, fig.width = 12}
...
skew_s <-ggplot(data = obs_sn, aes (x =nspp, y = skew_percentile)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d(end = .7) +
ggtitle("Skewness vs S")

skew_n <- ggplot(data = obs_sn, aes (x = nind, y = skew_percentile)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d(end = .7) +
  ggtitle("Skewness vs N")

skew_ns <- ggplot(data = obs_sn, aes (x = nind / nspp, y = skew_percentile)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d(end = .7) +
  ggtitle("Skewness vs N / S")

gridExtra::grid.arrange(grobs = list(skew_s, skew_n, skew_ns), nrow = 1)
```
...
all_skews_freqplot <- ggplot(data = filter(ranked_skew_df, source == "sampled"), aes(x = 0, y = skew)) +
  geom_violin() +
  theme_bw()
